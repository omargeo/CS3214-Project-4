...
/**
 * \brief Find a suitable IPv4 address to bind to, create a socket, bind it,
 * invoke listen to get the socket ready for accepting clients.
 * 
 * \param pinfo info of the addr to connect to 
 * \param backlog ??
 * \return int returns socket fd on success, reutrns -1 on fauiler 
 */
static int 
connect_socket(struct addrinfo* pinfo, int backlog)
{
    ...
    int rc = bind(s, pinfo->ai_addr, pinfo->ai_addrlen);
    if (rc == -1) {
        perror("bind");
        close(s);
        return -1;
...
/*
 * Find a suitable IPv6 address to bind to, create a socket, bind it,
 * invoke listen to get the socket ready for accepting clients. If no IPv6 is found 
 * it looks for an ipv4 address.
 *
...
socket_open_bind_listen(char * port_number_string, int backlog)
{
    ...
    struct addrinfo *pinfo_inet = NULL;
    ...
    int rc = getaddrinfo(NULL, port_number_string, &hint, &info); //gets list of structs with possible addrs
    if (rc != 0) {
    ...
    for (pinfo = info; pinfo; pinfo = pinfo->ai_next) {
        ...
        int s = connect_socket(pinfo, backlog);
        if(s == -1) return -1;
    ...
    if(pinfo_inet)
    {
        int s = connect_socket(pinfo_inet, backlog);
        if(s == -1) return -1;
        freeaddrinfo(info);
        return s;
